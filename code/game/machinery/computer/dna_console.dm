...
/obj/machinery/computer/scan_consolenew/proc/can_modify_occupant()
	...
	if(scanner_occupant.has_dna() && !HAS_TRAIT(scanner_occupant, TRAIT_RADIMMUNE) && !HAS_TRAIT(scanner_occupant, TRAIT_NOCLONE) && !isrobotic(scanner_occupant) || (connected_scanner.scan_level == 3))
		return TRUE
