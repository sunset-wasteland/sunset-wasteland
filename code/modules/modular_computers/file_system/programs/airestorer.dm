...
/datum/computer_file/program/aidiag/process_tick()
	...
	if(cardhold.flush)
		...
		return
	ai_slot.locked = TRUE
	A.adjustOxyLoss(-5, updating_health = FALSE)//, FALSE)
	A.adjustFireLoss(-5, updating_health = FALSE)//, FALSE)
	A.adjustToxLoss(-5, updating_health = FALSE)
	A.adjustBruteLoss(-5, updating_health = FALSE)
