...
/datum/disease/curling_thirteen/stage_act()
	...
	switch(stage)
		...
		if(2)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>You scratch at an itch.")
				affected_mob.adjustBruteLoss(3, updating_health = FALSE)
			if(prob(2))
				affected_mob.emote("cough")
				affected_mob.adjustOxyLoss(2, updating_health = FALSE)
		...
		if(3)
			...
			if(prob(3))
				...
				affected_mob.adjustOxyLoss(6, updating_health = FALSE)
				affected_mob.emote("gasp")
			if(prob(3))
				to_chat(affected_mob, "<span class='danger'>It hurts like hell! Make it stop!")
				affected_mob.adjustBruteLoss(6, updating_health = FALSE)
			if(prob(3))
