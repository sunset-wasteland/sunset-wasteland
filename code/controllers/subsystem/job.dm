...
/*		if(M.client.holder)
	...
	if(job)
		...
		if(job.roleplay_exclusive_notify)
			to_chat(M, "<b>You are playing a job that is exclusively for roleplay. In the event of a raid, you are not permitted to participate in either an attack or defence, and must despawn until it is over. Remain in your faction's base.</b>")
