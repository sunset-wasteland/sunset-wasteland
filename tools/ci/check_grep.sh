...
#    st=1
#fi;
if git grep -Pn '\td[1-2] =' _maps/**/*.dmm;	then
...
if git grep -Pn '^/area/.+[\{]' _maps/**/*.dmm;	then
...
if git grep -Pn '\W\/turf\s*[,\){]' _maps/**/*.dmm; then
...
if git grep -in 'vault[\s-]?tek' _maps/**/*.dmm; then
